...
class PunchcardUi(wx.Frame):
    def __init__(self, superion):
        wx.Frame.__init__(self, parent=superion, title="签到", size=(630, 620),)
        self.SetBackgroundColour('black')  # 修改背景色
        self.Center()
        image_file = r'.\icon\reco.png'
        to_bmp_image = wx.Image(image_file, wx.BITMAP_TYPE_ANY, -1).ConvertToBitmap()
        self.bitmap = wx.StaticBitmap(self, -1, to_bmp_image, (0, 0))
        self.OpenCapButton = wx.Button(self.bitmap,-1,pos=(282, 445), size=(65, 65), label='开始签到',style=wx.BORDER_NONE)
        font = wx.Font(12, wx.DECORATIVE, wx.BOLD, wx.BOLD)
        self.OpenCapButton.SetFont(font)
        self.OpenCapButton.SetForegroundColour('white')
        self.OpenCapButton.SetBackgroundColour('#122535')
        ...
        # # 显示图片
        box_image = r'.\icon\recotop.png'
        to_bmp_image = wx.Image(box_image, wx.BITMAP_TYPE_ANY, -1).ConvertToBitmap()
        self.bmp = wx.StaticBitmap(self, -1, to_bmp_image, (10, 10),size=(600, 320))
        self.OpenCapButton.SetBackgroundColour('#122535')
        self.SetBackgroundColour('black')  # 修改背景色
        self.image_fail = wx.Image(pun_fail, wx.BITMAP_TYPE_ANY).Scale(280, 200)
        self.image_repeat = wx.Image(pun_repeat, wx.BITMAP_TYPE_ANY).Scale(280, 200)
        self.image_success = wx.Image(pun_success, wx.BITMAP_TYPE_ANY).Scale(280, 200)
    ...
    def _open_cap(self, event):
        ...
        while self.cap.isOpened():
            ...
            # 设置检测识别窗口的大小
            # cv2.resizeWindow("im_rd", 500, 300)
            ...
            if len(dets) != 0:
                ...
                for i in range(len(features_known_arr)):
                    ...
                    if compare == "same":  # 找到了相似脸
                        ...
                        while i < len(recoders):
                            i = i + 1
                            # while j<len(recoders[i]):
                            #     recoders[i][j]
                            if i == 0:
                                if recoders[i][0] == recoder[0]:
                                    self.pun_day_num += 1
                                    # self.resultText.SetLabel("签到天数:" + str(self.pun_day_num))


                            if i == 0 :
                                if  recoders[i][0] == recoder[0] and recoders[1] == recoder[1]:
                        ...
                        with open(path_logcat_csv, "a+", newline="") as csvfilew:
                            ...
                            # strdate = recoder[1]+" "+recoder[2]

                            # start_date = date.strptime(strdate, '%Y/%m/%d')
                            # print(strdate)
                            print(date2)
                            from database import kaoqinwritedatabese
                            from database import Selectuid
                            id = Selectuid(name)

                            kaoqinwritedatabese(date2, name, id)
                ...
                # 写人脸名字
                cv2.putText(im_rd, name, pos, font, 0.5, (255, 0, 255), 1, cv2.LINE_AA)
                cv2.putText(im_rd, "Faces: " + str(len(dets)), (15, 25), font, 0.5, (255, 0, 0), 1, cv2.LINE_AA)
