...
# 保存
path_make_dir = "data/face_picture/"

path_feature_all = "data/face_all.csv"

info = 'icon/camera.png'
...
class   RegisterUi(wx.Frame):
    def __init__(self,superion):
        ...
        wx.Frame.__init__(self, parent=superion, title="人脸录入", size=(935, 680),style=wx.DEFAULT_FRAME_STYLE)
        ...
        # 管理员验证框
        while self.user == '':
            self.user = wx.GetTextFromUser(message="请先输入管理员密码", caption="温馨提示",
            ...
            from database import SelectDatabase

            admin = SelectDatabase(uid=k)

            for uuid in [1,0]:
                if admin == 1:
                    pass;
                else :
                    wx.MessageBox(message="管理员密码不对，请重新输入", caption="警告")
                    self.user = ''
                    break
        text = "1"
        image_file = r'.\icon\addback.png'

        # 处理存放所有人脸特征的csv
        path_feature_known_csv = "data/face_all.csv"
        ...
        facenum = str(len(features_known_arr))



        to_bmp_image = wx.Image(image_file, wx.BITMAP_TYPE_ANY, -1).ConvertToBitmap()
        self.bitmap = wx.StaticBitmap(self, -1, to_bmp_image, (0, 0))
        self.NewButton = wx.Button(self.bitmap,pos=(136,128),size=(26,26),label='入',style=wx.BORDER_NONE)
        self.ShortCutButton = wx.Button(self.bitmap,pos=(138,298),size=(26,26),label='存',style=wx.BORDER_NONE)
        self.SaveButton = wx.Button(self.bitmap,pos=(136,482),size=(26,26),label='入',style=wx.BORDER_NONE)
        self.facenumButton = wx.Button(self.bitmap,pos=(330,20),size=(30,30),label=facenum,style=wx.BORDER_NONE)
        # 按钮样式
        self.NewButton.SetForegroundColour('#18bce5')
        self.NewButton.SetBackgroundColour('#18bce5')
        self.ShortCutButton.SetForegroundColour('#eb833e')
        self.ShortCutButton.SetBackgroundColour('#eb833e')
        self.SaveButton.SetForegroundColour('#af72fb')
        self.SaveButton.SetBackgroundColour('#af72fb')
        self.facenumButton.SetForegroundColour('#18bce5')
        self.facenumButton.SetBackgroundColour('#070923')

        font = wx.Font(14, wx.DECORATIVE, wx.ITALIC, wx.NORMAL)
        self.facenumButton.SetFont(font)
        # 封面图片
        self.image_info = wx.Image(info, wx.BITMAP_TYPE_ANY).Scale(500, 480)
        # # 显示图片
        self.bmp = wx.StaticBitmap(parent=self, pos=(225,90))
        self.SetBackgroundColour('#070935')  # 修改背景色
    ...
    def OnNewButtonClicked(self, event):
        ...
        while self.id == ID_WORKER_UNAVIABLE:
            self.id = wx.GetNumberFromUser(message="请输入您的ID(-1不可用)",
                                           prompt="ID", caption="温馨提示",
            ...
            #         wx.MessageBox(message="id已存在，请重新输入", caption="警告")
            global  bitbit
            bitbit = self.id
            print(bitbit)
        ...
        while self.name == '':
            self.name = wx.GetTextFromUser(message="请先输入录入者的姓名,用于创建姓名文件夹", caption="温馨提示",default_value="", parent=None,)
        ...
        if self.register_flag == 0 and self.sc_number != 0:
            ...
            if len(feature_list) > 0:
                ...
                with open(path_feature_all, "a+", newline="") as csvfile:
                    ...
                    wx.MessageBox(message="人脸录入完成", caption="提示")

                    from database import encode
                    feature_average_base64 = encode(feature_average)
                    #base64编码
                    from database import facewritedatabese
                    facewritedatabese(feature_average_base64,self.name,self.id)
                    # 写进数据库
                    from database import infowritedatabese
                    infowritedatabese(self.id,self.name)
